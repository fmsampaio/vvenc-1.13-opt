...
VVENC_OUTPUTS_FOLDER = f'{OUTPUTS_FOLDER}/vvenc-reports'
    ...
    for video in VIDEOS:
        for qp in QPs:
            for config in CONFIGS:
                ...
                vvencCommand = f'{VVENC_CMD} -c {ENCODER_CFG}/{config[1]} -c {CFG_VIDEOS_FOLDER}/{video[0]}.cfg -i {YUV_VIDEOS_FOLDER}/{video[1]} -q {qp} -f {FRAMES_TO_BE_CODED} -b {ENCODED_OUTPUTS_FOLDER}/{experimentKey}.bin'
                vvencReportFile = f'{VVENC_OUTPUTS_FOLDER}/{experimentKey}.report'

                manager.addExecution(
                    key = experimentKey,
                    command = vvencCommand,
                    outputFileName = vvencReportFile
                )
